...
public class JdbcGuestRepository implements GuestRepository {
    ...
    public JdbcGuestRepository(JdbcAttractionRepository attractionRepository) {
        ...
        try {
            ...
            ResultSet resultSet = databaseMetaData.getTables(null, null, "GUEST",
                    new String[] {"TABLE"});
            if(resultSet.next()){    // table exists
                manager.getTransaction().begin();
                Query query = manager.createNativeQuery("DROP TABLE guest", Guest.class);
                query.getResultList();
    ...
    private void populateGuests(){
        ...
        attractionRepository.getInstructorRepository().update(instr1, instr1.getID());
        attractionRepository.getInstructorRepository().update(instr2, instr2.getID());
    ...
    @Override
    public void update(Guest guest, String idGuest){
