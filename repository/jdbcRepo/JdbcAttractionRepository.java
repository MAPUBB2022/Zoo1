...
public class JdbcAttractionRepository implements AttractionRepository {
    ...
    public JdbcAttractionRepository(JdbcInstructorRepository instructorRepository) {
        ...
        try {
            ...
            ResultSet resultSet = databaseMetaData.getTables(null, null, "ATTRACTION",
                    new String[] {"TABLE"});
            if(resultSet.next()){    // table exists
                manager.getTransaction().begin();
                Query query = manager.createNativeQuery("DROP TABLE attraction", Guest.class);
                query.getResultList();
    ...
    private void populateAttractions(){
        ...
        instructorRepository.update(instructor1, instructor1.getID());
        instructorRepository.update(instructor2, instructor2.getID());
        instructorRepository.update(instructor3, instructor3.getID());
        instructorRepository.update(instructor4, instructor4.getID());
        instructorRepository.update(instructor5, instructor5.getID());
        instructorRepository.update(instructor6, instructor6.getID());
    ...
    @Override
    public void update(Attraction attraction, String idAttraction){
